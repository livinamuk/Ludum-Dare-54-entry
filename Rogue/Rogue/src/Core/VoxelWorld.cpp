...
std::vector<Vertex> _collisionLineVertices;
...
void BeginNewBackFacingZSearch(int xStart, int yStart, int z) {
    ...
    if (yMin <= 0.1f) {
        Vertex a, b;
        a.position = { xMin, yMin, zConstant };
        b.position = { xMax, yMin, zConstant };
        _collisionLineVertices.push_back(a);
        _collisionLineVertices.push_back(b);
    }
}
...
void BeginNewFrontFacingXSearch(int x, int yStart, int zStart) {
    ...
    if (yMin <= 0.1f) {
        Vertex a, b;
        a.position = { xConstant, yMin, zMin };
        b.position = { xConstant, yMin, zMax };
        _collisionLineVertices.push_back(a);
        _collisionLineVertices.push_back(b);
    }
}
...
std::vector<Vertex>& VoxelWorld::GetCollisionLineVertices() {
    return _collisionLineVertices;
}
